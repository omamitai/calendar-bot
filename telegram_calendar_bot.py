...
def _jsave(p, d):
    try: p.write_text(json.dumps(d, ensure_ascii=False, indent=2), "utf-8")
    except Exception as e: log.error("save %s: %s", p, e)

def get_cal(uid):  return _jload(USERS_FILE).get(str(uid))
def set_cal(uid, cid):
    d = _jload(USERS_FILE); d[str(uid)] = cid; _jsave(USERS_FILE, d)

def push_hist(uid, eid, cid, data):
    h = _jload(HIST_FILE); k = str(uid)
    h.setdefault(k, []).append({"eid": eid, "cid": cid, "data": data})
    h[k] = h[k][-30:]
    _jsave(HIST_FILE, h)
...
def pop_hist(uid):
    ...
    ev = lst.pop(); _jsave(HIST_FILE, h); return ev
...
def init_google():
    ...
    p = Path("/tmp/sa.json"); p.write_bytes(base64.b64decode(b64))
...
def gcal_del(svc, cid, eid):
    try: svc.events().delete(calendarId=cid, eventId=eid).execute()
    except: pass
...
def init_gemini():
    ...
    return genai.GenerativeModel(GEMINI_MODEL, generation_config={"temperature": GEMINI_TEMP, "max_output_tokens": 512})
...
def _find_json(raw: str) -> dict:
    ...
    raise ValueError(f"No JSON in: {raw[:80]}")
...
async def _call_gemini(model, content, timeout=LLM_TIMEOUT, retries=LLM_RETRIES):
    ...
    for attempt in range(retries + 1):
        try:
            ...
            r = await asyncio.wait_for(loop.run_in_executor(None, fn), timeout=timeout)
            if r and r.text: return r.text
        except asyncio.TimeoutError:
            log.warning("Gemini timeout (attempt %d)", attempt + 1)
        except Exception as e:
            log.warning("Gemini error (attempt %d): %s", attempt + 1, e)
        if attempt < retries:
...
def normalize(ev: dict) -> dict:
    ...
    if not ev["start_time"]:
        ...
        ev["is_all_day"] = bool(ev.get("is_all_day"))
    return ev


def validate(ev: dict) -> str | None:
    """Returns error message or None."""
    if not ev.get("title"): return "חסרה כותרת"
    if not ev.get("date"):  return "חסר תאריך"
    try: datetime.strptime(ev["date"], "%Y-%m-%d")
    except: return "תאריך לא תקין"
    return None
...
_DAY = {"Sunday":"ראשון","Monday":"שני","Tuesday":"שלישי","Wednesday":"רביעי","Thursday":"חמישי","Friday":"שישי","Saturday":"שבת"}
_MON = {1:"ינואר",2:"פברואר",3:"מרץ",4:"אפריל",5:"מאי",6:"יוני",7:"יולי",8:"אוגוסט",9:"ספטמבר",10:"אוקטובר",11:"נובמבר",12:"דצמבר"}
...
def _htime(ev):
    if ev.get("is_all_day") or not ev.get("start_time"): return "כל היום"
    s = ev["start_time"]
...
def _rel(ds):
    try:
        ...
        if d == t: return "  · היום"
        if d == t + timedelta(1): return "  · מחר"
        if d == t + timedelta(2): return "  · מחרתיים"
        diff = (d - t).days
        if 0 < diff <= 7:
...
def kb_edit(ev):
    ...
    if ev.get("location"):
        row1.append(InlineKeyboardButton("מיקום", callback_data="e:loc"))
    return InlineKeyboardMarkup([
        row1,
        [
...
def build_app(svc, sa_email, mdl):
    ...
    async def cmd_start(update: Update, ctx: ContextTypes.DEFAULT_TYPE) -> int:
        ...
        if get_cal(uid):
            await update.message.reply_text(
                ...
                f"{R}שלח אירוע או תמונה.\n\n"
                f"{R}/setup לשינוי יומן  ·  /help לעזרה",
                parse_mode=ParseMode.HTML)
            return ConversationHandler.END
        await update.message.reply_text(
            f"{R}👋  <b>שלום{(' ' + name) if name else ''}!</b>\n\n"
            f"{R}אני מוסיף אירועים ל-Google Calendar —\n"
            f"{R}כתוב בעברית או שלח צילום מסך.\n\n"
            f"─────────────────────\n\n"
            f"{R}<b>הגדרה חד-פעמית:</b>\n\n"
            f"{R}<b>①</b>  שתף את היומן עם:\n"
            f"<code>{sa_email}</code>\n\n"
            f"{R}<i>Calendar → ⚙️ ליד היומן → שיתוף</i>\n"
            f"{R}<i>→ הרשאה: ביצוע שינויים</i>\n\n"
            f"{R}<b>②</b>  שלח את כתובת ה-Gmail:",
            parse_mode=ParseMode.HTML)
        return ST_CAL
    ...
    async def recv_cal(update: Update, ctx: ContextTypes.DEFAULT_TYPE) -> int:
        ...
        if "@" not in text:
            await update.message.reply_text(f"{R}שלח כתובת Gmail, למשל:\n<code>name@gmail.com</code>", parse_mode=ParseMode.HTML)
            return ST_CAL
        await ctx.bot.send_chat_action(update.effective_chat.id, ChatAction.TYPING)
        try: svc.calendarList().insert(body={"id": text}).execute()
        except: pass
        try:
            svc.events().list(calendarId=text, maxResults=1, singleEvents=True).execute()
            ...
            await update.message.reply_text(
                ...
                parse_mode=ParseMode.HTML)
            return ST_CAL
        set_cal(update.effective_user.id, text)
        await update.message.reply_text(
            f"{R}✅  <b>מחובר!</b>\n\n{R}שלח אירוע או תמונה.",
            parse_mode=ParseMode.HTML)
        return ConversationHandler.END
    ...
    async def cmd_help(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
        await update.message.reply_text(
            ...
            f"{R}<b>✏️  עריכה</b> — לחץ ✏️ אחרי הוספה\n"
    ...
    async def cmd_undo(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
        ...
        await update.message.reply_text(card_del(ev["data"].get("title","?")), parse_mode=ParseMode.HTML)
    ...
    async def _process(uid, ev, msg):
        ...
        cid = get_cal(uid)
        if not cid:
            await _safe_edit(msg, f"{R}שלח /start כדי לחבר.", parse_mode=ParseMode.HTML)
            return
        ev = normalize(ev)
        err = validate(ev)
        if err:
            await _safe_edit(msg, f"{R}⚠️  {err}\n{R}נסה שוב בניסוח אחר.", parse_mode=ParseMode.HTML)
            return
        try:
            created = gcal_insert(svc, cid, ev)
        except Exception as exc:
            log.exception("Calendar error")
            await _safe_edit(msg, f"{R}⚠️  שגיאה ביומן:\n<code>{str(exc)[:80]}</code>", parse_mode=ParseMode.HTML)
            return
        push_hist(uid, created["id"], cid, ev)
        await _safe_edit(msg,
            card_ok(ev, created.get("htmlLink","")),
            parse_mode=ParseMode.HTML, reply_markup=kb_done())
    ...
    async def handle_text(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
        ...
        try:
            ...
            await _safe_edit(msg,
                f"{R}לא הצלחתי לפענח.\n{R}נסה: <i>\"פגישה עם דני מחר ב-10\"</i>",
                parse_mode=ParseMode.HTML)
            return
    ...
    async def _handle_img(update, ctx, img_bytes, caption):
        ...
        try:
            ...
            await _safe_edit(msg, f"{R}לא זיהיתי אירוע בתמונה.\n{R}נסה תמונה ברורה יותר או כתוב בטקסט.")
            return
        await _process(uid, ev, msg)
    ...
    async def handle_doc(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
        ...
        if not d.mime_type or not d.mime_type.startswith("image/"): return
    ...
    async def cb_del(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
        ...
        if ed:
            # In edit mode — delete original from calendar
            if orig:
                gcal_del(svc, orig["cid"], orig["eid"])
                pop_hist(uid)
            await q.edit_message_text(card_del(ed.get("title","?")), parse_mode=ParseMode.HTML)
            return

        ev = pop_hist(uid)
        if not ev:
            await _safe_edit(q.message, f"{R}אין מה לבטל.", parse_mode=ParseMode.HTML)
            return
        gcal_del(svc, ev["cid"], ev["eid"])
        await q.edit_message_text(card_del(ev["data"].get("title","?")), parse_mode=ParseMode.HTML)

    async def cb_edit_start(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
        q = update.callback_query; await q.answer()
        uid = q.from_user.id
        h = _jload(HIST_FILE).get(str(uid), [])
        if not h:
            await _safe_edit(q.message, f"{R}אין אירוע לעריכה.", parse_mode=ParseMode.HTML)
            return
        last = h[-1]
        ev = dict(last["data"])
        ctx.user_data["editing"] = ev
        ctx.user_data["edit_orig"] = {"eid": last["eid"], "cid": last["cid"]}
        ctx.user_data["edit_msg_id"] = q.message.message_id
        await q.edit_message_text(card_edit(ev), parse_mode=ParseMode.HTML, reply_markup=kb_edit(ev))
    ...
    async def cb_edit_field(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
        ...
        await q.message.reply_text(f"{R}✏️  {hints.get(field, 'שלח ערך:')}")
    ...
    async def _apply_edit(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
        ...
        if field == "title":
            ...
            if t:
                ev["start_time"] = t; ev["is_all_day"] = False
                ev["end_time"] = _norm_time(parts[1]) if len(parts) > 1 else None
            else:
                await update.message.reply_text(f"{R}פורמט: <code>10:00</code> או <code>14:00-16:00</code>", parse_mode=ParseMode.HTML)
                ctx.user_data["edit_field"] = field; ok = False
        elif field == "date":
            ...
            if m:
                ...
                if d:
                    ...
                    await update.message.reply_text(f"{R}לא הבנתי. נסה: <code>25.3</code> / מחר", parse_mode=ParseMode.HTML)
                    ctx.user_data["edit_field"] = field; ok = False
            ...
            if mid:
                try:
                    await ctx.bot.edit_message_text(
                        ...
                        text=card_edit(ev), parse_mode=ParseMode.HTML, reply_markup=kb_edit(ev))
                except: pass
    ...
    async def cb_save(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
        ...
        ctx.user_data.pop("edit_field", None)
        if not ev:
            await _safe_edit(q.message, f"{R}אין שינויים.", parse_mode=ParseMode.HTML)
            return
        # Delete original
        if orig:
            gcal_del(svc, orig["cid"], orig["eid"])
        ...
        cid = get_cal(uid)
        if not cid:
            await _safe_edit(q.message, f"{R}שלח /start.", parse_mode=ParseMode.HTML)
            return
        ev = normalize(ev)
        err = validate(ev)
        if err:
            await _safe_edit(q.message, f"{R}⚠️  {err}", parse_mode=ParseMode.HTML)
            return
        try:
            created = gcal_insert(svc, cid, ev)
        except Exception as exc:
            await _safe_edit(q.message, f"{R}⚠️  {str(exc)[:80]}", parse_mode=ParseMode.HTML)
            return
        push_hist(uid, created["id"], cid, ev)
    ...
    async def on_error(update, ctx: ContextTypes.DEFAULT_TYPE):
        if isinstance(ctx.error, (TimedOut, NetworkError)):
            ...
            return
        log.error("Error: %s\n%s", ctx.error, traceback.format_exception(ctx.error))
    ...
    app.add_handler(ConversationHandler(
        ...
        states={ST_CAL: [MessageHandler(filters.TEXT & ~filters.COMMAND, recv_cal)]},
    ...
    app.add_handler(CallbackQueryHandler(cb_edit_field,  pattern="^e:(title|date|time|loc)$"))
